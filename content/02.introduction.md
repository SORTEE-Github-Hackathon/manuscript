...
A basic workflow using git and GitHub is to 1) make changes to files on your computer, 2) record related changes in files as a "commit" (see Box 1) along with a message describing the changes, 3) synchronize commits with GitHub (see "push" in Box 1) so that the project (known as a "repository") is up to date. Commits act like snapshots and one can revisit the state of the project to any commit to view or undo changes.
