...
GitHub is not reccomended for storing large files and thus, commits (see [Box 1](#definitions)]) greater than 50 MB receive a warning and commits larger than 100 MB are blocked (@https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-large-files-on-github). 
Therefore, it is reccomended that large datasets are stored externally, with services such as Zenodo or other data repositories. These services may have their own file storage limits (e.g. Zenodo has a limit of 50GB per dataset) but allow users to upload multiple datasets, and often offer additional storage services for large datasets with an associated cost (@https://git-lfs.github.com, @https://help.zenodo.org).
<!-- Condensed some of this text to make more concise  -->
