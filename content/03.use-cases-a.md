...
In EEB, a research compendium includes all research products including data, code for analyses, and other metadata that make code and data reusable.
...
Many researchers often start their use of GitHub to backup their working code and data to a remote server (pull-commit-push, see [Box 1](#definitions), from their own repo). 
...
Although it is not recommended to use GitHub for long term data archiving because GitHub is a privately held company that does not guarantee permanent storage, GitHub integrates with Zenodo, a CERN-funded long-term data archiving service. Zenodo can be linked to your GitHub account and archival activated for any public repository. Then, every time a release ([Box 1](#definitions)) is made, a snapshot of the entire repository is archived in Zenodo with a versioned, citable DOI that can be mentioned in a Data Availability Statement (see 'Making code citable' below for further discussion).
...
Therefore, it is recommended that large datasets or output files are stored externally, with services such as Zenodo or other data repositories. 
These services may have their own file storage limits (e.g. Zenodo has a limit of 50 GB per dataset) but allow users to upload multiple datasets, and often offer additional storage services for large datasets with an associated cost (@https://git-lfs.github.com, @https://help.zenodo.org).
