...
GitHub has a 100 Mb (megabyte) limit for committed files, which might require large files (such as data) to be stored externally to the GitHub repository (e.g. in a different directory on your computer or using cloud-hosting). With some additional setup, these external files can still be tracked in the GitHub repository using Git Large File Storage (<https://git-lfs.github.com>).
