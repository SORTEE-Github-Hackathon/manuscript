...
First, real-time collaborative editing of files is still not possible in GitHub.
...
Although creating tables and figures can be achieve through code, users may be limited to other software to collaboratively brainstorm figures and tables (_e.g_. Google Slides, Google Sheets; but see GitHub Discussions).
...
However, the acquisition of GitHub by Microsoft has driven concerns over the future of free plans, causing several biodiversity data managers to shift to alternative Open Source Git services (_e.g._, Bitbucket and GitLab).
